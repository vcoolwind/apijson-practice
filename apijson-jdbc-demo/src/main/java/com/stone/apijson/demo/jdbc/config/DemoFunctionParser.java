...
public class DemoFunctionParser extends APIJSONFunctionParser {
     ...
     * @throws Exception
     */
    //TODO 这里绑定了本对象，并不是一个很好的方案，好的方案是通过bean对象+方法进行，一方面避免在一个类实现过多的方法，也避免了不支持方法重名。
    ...
    public Object isUserCanPutMoment(@NotNull JSONObject current, @NotNull String momentId) throws Exception {
        Object idObj = current.get(momentId);
        Object idListObj = current.get(momentId + "{}");
        if (idObj instanceof Number) {      // PUT 单个请求
            // continue
            if(idObj.equals(123)){
                //测试数据
                return null;
        ...
        Visitor<Long> visitor = getVisitor(this.getSession());
        if (visitor == null) {
            throw new IllegalAccessException("user not logged in");
        }
        ...
        // 内部错误
        if (!JSONResponse.isSuccess(jsonResponse)) {
            return APIJSONParser.extendErrorResult(jsonResponse, new RuntimeException("inner error"));
        }
        // 如果不存在的话
        if (!jsonResponse.containsKey(momentClzName)) {
            return APIJSONParser.newErrorResult(new ConditionErrorException("moment not exist"));
        }
    ...
    public Object isUserCanPutTodo(@NotNull JSONObject current, @NotNull String todoId) throws Exception {
        ...
        for (Long TodoId : todoIdList) {
            ...
            todoRequest.put(UserController.USER_CLASS_NAME, userRequest);
